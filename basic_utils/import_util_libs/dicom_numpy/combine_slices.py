...
def combine_slices(datasets, rescale=None):
    ...
    """
    slice_datasets = [ds for ds in datasets if not _is_dicomdir(ds)]  # + Check whether it is dicom files

    if len(slice_datasets) == 0:
        raise DicomImportException("Must provide at least one image DICOM dataset")

    _validate_slices_form_uniform_grid(slice_datasets)  # + Check whether data got uniform infos

    voxels = _merge_slice_pixel_arrays(slice_datasets, rescale)
    transform, direction, spacing = _ijk_to_patient_xyz_transform_matrix(slice_datasets)

    return voxels, transform, direction, spacing
...
def _merge_slice_pixel_arrays(slice_datasets, rescale=None):
    sorted_slice_datasets = _sort_by_slice_position(slice_datasets)  # + Sort slices in order

    if rescale is None:
        rescale = any(_requires_rescaling(d) for d in sorted_slice_datasets)  # + If scale is none, use the scale info from the data

    first_dataset = sorted_slice_datasets[0]
    slice_dtype = first_dataset.pixel_array.dtype
    slice_shape = first_dataset.pixel_array.T.shape  # + Shape of (x, y)
    num_slices = len(sorted_slice_datasets)  # + Size of z

    voxels_shape = slice_shape + (num_slices,)  # Get Matrix Size (x, y, z)
    ...
    for k, dataset in enumerate(sorted_slice_datasets):
        pixel_array = dataset.pixel_array.T  # ! Pixel data transposed here!!!!!
        if rescale:
            slope = float(getattr(dataset, 'RescaleSlope', 1))
            intercept = float(getattr(dataset, 'RescaleIntercept', 0))
            pixel_array = pixel_array.astype(np.float32) * slope + intercept  # + Seems similar as my code
...
def _ijk_to_patient_xyz_transform_matrix(slice_datasets):
    first_dataset = _sort_by_slice_position(slice_datasets)[0]  # + Sort data
    image_orientation = first_dataset.ImageOrientationPatient  # + Get orientation of the first slice
    row_cosine, column_cosine, slice_cosine = _extract_cosines(image_orientation)  # Use cross to get slice vec

    row_spacing, column_spacing = first_dataset.PixelSpacing  # + Get spacing of each pixel (x, y)
    slice_spacing = _slice_spacing(slice_datasets)  # + Use the location of the upper left hand corner projected on slice vec

    transform = np.identity(4, dtype=np.float32)  # + Get a matrix with trace all 1

    transform[:3, 0] = row_cosine * column_spacing  # + column_spacing, etc. is like ratio
    ...
    return transform, (row_cosine, column_cosine, slice_cosine), \
           np.asarray([float(column_spacing), float(row_spacing), float(slice_spacing)])
...
def _slice_positions(slice_datasets):
    ...
    row_cosine, column_cosine, slice_cosine = _extract_cosines(image_orientation)
    return [np.dot(slice_cosine, d.ImagePositionPatient) for d in slice_datasets]  # + The x, y, and z coordinates of the upper left hand corner
...
def _slice_spacing(slice_datasets):
    if len(slice_datasets) > 1:
        slice_positions = _slice_positions(slice_datasets)  # + Get projected position of the upper left hand corner on the slice vec
        slice_positions_diffs = np.diff(sorted(slice_positions))  # + Get diff of every 2 points
