...
def compute_grid_modified(image_size, upper_limit=128, dtype=th.float32, device='cpu'):

    dim = len(image_size)

    if dim == 2:
        raise ValueError('This code is not useful.')
        ...
        x = th.linspace(-1, 1, steps=nx).to(dtype=dtype) * nx / upper_limit
        y = th.linspace(-1, 1, steps=ny).to(dtype=dtype) * ny / upper_limit
        z = th.linspace(-1, 1, steps=nz).to(dtype=dtype) * nz / upper_limit
...
def warp_image(image, displacement):
    ...
    grid = compute_grid([128, 128, 128], dtype=image.dtype, device=image.device)  # + get the -1 to 1 grid

    displacement = displacement + grid
    displacement *= (128 / th.as_tensor(image.size, device=image.device))

    # warp image
    warped_image = F.grid_sample(image.image, displacement)
