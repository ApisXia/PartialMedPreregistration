...
class RigidTransformation(_Transformation):
    ...
    """
    def __init__(self, moving_image, opt_cm=False, init_3dra_matrix=None, init_ct_matrix=None):
        super(RigidTransformation, self).__init__(image_size=[128, 128, 128],
                                                  dtype=moving_image.dtype,
                                                  device=moving_image.device)

        self._opt_cm = opt_cm  # ? dont know this

        grid = th.squeeze(tu.compute_grid_modified(moving_image.size, dtype=self._dtype))  # + compute position of pixels from [-1, 1]

        grid = th.cat((grid, th.ones(*[list(moving_image.size) + [1]], dtype=self._dtype)), self._dim)\
               .to(device=self._device)  # + add extra channel 1 for matrix transformation

        self.register_buffer("_grid_moving", grid)

        # compute the initial center of mass of the moving image
        intensity_sum = th.sum(moving_image.image)

        self._center_mass_x = th.sum(moving_image.image.squeeze() * self._grid_moving[..., 0]) / intensity_sum  # + ? similar way in physics to get the center of image
        self._center_mass_y = th.sum(moving_image.image.squeeze() * self._grid_moving[..., 1]) / intensity_sum


        self._phi_z = Parameter(th.tensor(0.0))  # + define rotation and translation for 2D images
        self._t_x = Parameter(th.tensor(0.0))
        self._t_y = Parameter(th.tensor(0.0))

        self._trans_matrix_pos = None  # ? I dont know
        self._trans_matrix_cm = None
        self._trans_matrix_cm_rw = None
        self._rotation_matrix = None

        if init_3dra_matrix is None:
            init_3dra_matrix = np.diag(np.ones(4))
        if init_ct_matrix is None:
            init_ct_matrix = np.diag(np.ones(4))

        self._init_matrix = th.mm(th.from_numpy(init_3dra_matrix).to(dtype=self._dtype, device=self._device),
                                  th.from_numpy(init_ct_matrix).to(dtype=self._dtype, device=self._device))

        if self._opt_cm:
            self._center_mass_x = Parameter(self._center_mass_x)  # ? Maybe means allow center of mass to be updated
        ...
        if self._dim == 2:
            ...
            self._compute_transformation = self._compute_transformation_3d  # ? a function return transformation_matrix based on phixyz?

            self._center_mass_z = th.sum(moving_image.image.squeeze() * self._grid_moving[..., 2]) / intensity_sum  # + another center of mass
    ...
    def init_translation(self, fixed_image):
        ...
        grid = th.squeeze(
            tu.compute_grid_modified(fixed_image.size, dtype=self._dtype))  # + compute position of pixels from [-1, 1]

        grid = th.cat((grid, th.ones(*[list(fixed_image.size) + [1]], dtype=self._dtype)), self._dim) \
            .to(device=self._device)  # + add extra channel 1 for matrix transformation

        self.register_buffer("_grid_fixed", grid)

        # + calculate the center of mass of the fixed image
        fixed_image_center_mass_x = th.sum(fixed_image.image.squeeze() * self._grid_fixed[..., 0]) / intensity_sum
        fixed_image_center_mass_y = th.sum(fixed_image.image.squeeze() * self._grid_fixed[..., 1]) / intensity_sum

        # self._t_x = Parameter(self._center_mass_x - fixed_image_center_mass_x)  # + used to init the translation parameters
        self._t_x = Parameter(th.as_tensor(0, dtype=self._center_mass_x.dtype, device=self._center_mass_x.device))  # + used to init the translation parameters
        # self._t_y = Parameter(self._center_mass_y - fixed_image_center_mass_y)
        self._t_y = Parameter(th.as_tensor(0, dtype=self._center_mass_y.dtype, device=self._center_mass_y.device))

        if self._dim == 3:
            fixed_image_center_mass_z = th.sum(fixed_image.image.squeeze() * self._grid_fixed[..., 2]) / intensity_sum
            # self._t_z = Parameter(self._center_mass_z - fixed_image_center_mass_z)
            self._t_z = Parameter(th.as_tensor(0, dtype=self._center_mass_z.dtype, device=self._center_mass_z.device))
    ...
    def _compute_transformation_3d(self):
        ...
        self._trans_matrix_pos = th.diag(th.ones(self._dim + 1, dtype=self._dtype, device=self._device))
        self._trans_matrix_cm = th.diag(th.ones(self._dim + 1, dtype=self._dtype, device=self._device))
        self._trans_matrix_cm_rw = th.diag(th.ones(self._dim + 1, dtype=self._dtype, device=self._device))  # + define 3 diagnal matrix

        self._trans_matrix_pos[0, 3] = self._t_x  # + based on difference of center of mass
        self._trans_matrix_pos[1, 3] = self._t_y
        self._trans_matrix_pos[2, 3] = self._t_z

        self._trans_matrix_cm[0, 3] = -self._center_mass_x  # + based on reversed center of mass
        self._trans_matrix_cm[1, 3] = -self._center_mass_y
        self._trans_matrix_cm[2, 3] = -self._center_mass_z

        self._trans_matrix_cm_rw[0, 3] = self._center_mass_x  # + based on center of mass
        ...
        self._rotation_matrix = th.mm(th.mm(R_z, R_y), R_x)  # + based on phixyz

    def _compute_transformation_matrix(self):
        # + key! this transformation is 1. translate to center or mass; 2. rotate; 3. translate back to original; 4. translate to the center of mass of fixed image
        ...
        # transformation_matrix = th.mm(self._trans_matrix_pos, self._rotation_matrix)[0:self._dim, :]
        transformation_matrix = th.mm(th.mm(self._trans_matrix_pos, self._rotation_matrix), self._init_matrix)[0:self._dim, :]
        return transformation_matrix

    def _compute_dense_flow(self, transformation_matrix):

        displacement = th.mm(self._grid_fixed.view(np.prod(self._image_size).tolist(), self._dim + 1),
                             transformation_matrix.t()).view(*(self._image_size.tolist()), self._dim) \
                       - self._grid_fixed[..., :self._dim]  # + key! get displacement of each pt after applying transformation
        return displacement
    ...
    def forward(self):
        ...
        self._compute_transformation()  # ? compute 3 translation matrix and 1 rotation matrix
        transformation_matrix = self._compute_transformation_matrix()  # + get a combined transformation matrix
        flow = self._compute_dense_flow(transformation_matrix)  # + get the description of the flow of each pt

        return self._concatenate_flows(flow)  # ? this constant flow is not used
