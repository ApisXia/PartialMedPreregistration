...
class _PairwiseImageLoss(th.nn.modules.Module):
    def __init__(self, fixed_image, moving_image, fixed_mask=None, moving_mask=None, size_average=True, reduce=True):
        ...
        self._grid = T.utils.compute_grid_modified(self._fixed_image.size, dtype=self._fixed_image.dtype,
                                     device=self._fixed_image.device)
...
class MSE(_PairwiseImageLoss):
    ...
    def forward(self, displacement):
        ...
        displacement = self._grid + displacement  # + flow back to displacement (redundant procedure)

        displacement *= (128 / th.as_tensor(self._moving_image.size, device=self._device))

        # compute current mask
        mask = super(MSE, self).GetCurrentMask(displacement)  # + key! get mask of those pts outside of images

        # warp moving image with displacement field
        self.warped_moving_image = F.grid_sample(self._moving_image.image, displacement, padding_mode="zeros")  # + It should aligned

        # + compute squared differences (a simple computation)
        value = (self.warped_moving_image - self._fixed_image.image).pow(2)

        # mask values
        value = th.masked_select(value, mask)  # + get values selected by mask
...
class MI(_PairwiseImageLoss):
    ...
    """
    def __init__(self, fixed_image, moving_image, fixed_mask=None, moving_mask=None, bins=64, sigma=25,
        ...
        if background is None:  # + set min or mean of images as background
        ...
        self._bins_fixed_image = th.linspace(self._background_fixed, self._max_f, self.bins,
                                             device=fixed_image.device, dtype=fixed_image.dtype).unsqueeze(1)  # + bins based on min and max of images
    ...
    def _compute_marginal_entropy(self, values, bins):
        ...
        p = th.exp(-((values - bins).pow(2).div(self._sigma))).div(self._normalizer_1d)  # + todo: very weird marginal entropy
        p_n = p.mean(dim=1)
    ...
    def forward(self, displacement):
        ...
        self._warped_moving_image = F.grid_sample(self._moving_image.image, displacement)
        # + till this everything is the same

        moving_image_valid = th.masked_select(self._warped_moving_image, mask)  # + use mask to get image pts overlapped
        fixed_image_valid = th.masked_select(self._fixed_image.image, mask)

        mask = (fixed_image_valid > self._background_fixed) & (moving_image_valid > self._background_moving)  # + get mask both image are not background
        ...
        sample = th.zeros(number_of_pixel, device=self._fixed_image.device,
                          dtype=self._fixed_image.dtype).uniform_() < self._spatial_samples  # + using uniform sampling to get part of pts

        # compute marginal entropy fixed image
        image_samples_fixed = th.masked_select(fixed_image_valid.view(-1), sample)
        if len(image_samples_fixed) == 0:
            ent_fixed_image, p_f = self._compute_marginal_entropy(image_samples_fixed, self._bins_fixed_image)
        else:
            bins_fixed_image = th.linspace(th.min(image_samples_fixed), th.max(image_samples_fixed), self.bins,
                                                 device=image_samples_fixed.device, dtype=image_samples_fixed.dtype).unsqueeze(1)
            ent_fixed_image, p_f = self._compute_marginal_entropy(image_samples_fixed, bins_fixed_image)

        # compute marginal entropy moving image
        image_samples_moving = th.masked_select(moving_image_valid.view(-1), sample)
        if len(image_samples_moving) == 0:
            ent_moving_image, p_m = self._compute_marginal_entropy(image_samples_moving, self._bins_moving_image)
        else:
            bins_moving_image = th.linspace(th.min(image_samples_moving).detach(), th.max(image_samples_moving).detach(), self.bins,
                                           device=image_samples_moving.device, dtype=image_samples_moving.dtype).unsqueeze(1)
            ent_moving_image, p_m = self._compute_marginal_entropy(image_samples_moving, bins_moving_image)
        ...
        ##### min
        # NMI = (ent_fixed_image + ent_moving_image - ent_joint) / (th.min(ent_fixed_image, ent_moving_image))
        ##### max
        # NMI = (ent_fixed_image + ent_moving_image - ent_joint) / (th.max(ent_fixed_image, ent_moving_image))
        ##### square average
        # NMI = (ent_fixed_image + ent_moving_image - ent_joint) / th.sqrt((ent_fixed_image**2 + ent_moving_image**2) / 2)
        ##### original paper
        NMI = (ent_fixed_image + ent_moving_image) / (ent_joint + 1e-10)
        return -NMI
        #
...
class SSIM(_PairwiseImageLoss):
    ...
    def forward(self, displacement):
        ...
        mean_moving_image = F.conv3d(self._warped_moving_image, self._kernel)

        variance_moving_image = F.conv3d(self._warped_moving_image.pow(2), self._kernel) - (
            mean_moving_image.pow(2))

        mean_fixed_moving_image = F.conv3d(self._fixed_image.image * self._warped_moving_image, self._kernel)
