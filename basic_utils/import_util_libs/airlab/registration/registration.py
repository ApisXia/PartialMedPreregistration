...
class PairwiseRegistration(_PairwiseRegistration):
    ...
    def _closure(self):  # + key key key code for calculate loss, and optimize
        self._optimizer.zero_grad()

        displacement = self._transformation()  # + get flow of each pt

        # compute the image loss
        lossList = []
        loss_names = []
        for image_loss in self._image_loss:  # + got 2 kinds
        ...
        if self._verbose:
            for loss_value, loss_name in zip(lossList, loss_names):
                self._print_buffer.append(str(loss_name) + ": " + str(loss_value.data.item()))
    ...
    def start(self, EarlyStopping=False, StopPatience=10):
        ...
        with tqdm(range(self._number_of_iterations)) as desc:
            for iter_index in desc:
                self._print_buffer = []
                if self._verbose:  # + whether print
                    self._print_buffer.append(str(iter_index))
                loss = self._optimizer.step(self._closure)
                desc.set_postfix_str(' '.join(self._print_buffer))
                if EarlyStopping:
                    if loss < self.loss:
                        n = 0
                        self.loss=loss
