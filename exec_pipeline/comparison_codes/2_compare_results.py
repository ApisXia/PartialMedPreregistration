...
def analyze_outputs_save(base_path):
    ...
    for i, ori_case in enumerate(ori_case_list):
        ...
        _, or_matrix, _, _ = load_dcm(ori_case)
        gt_pixels, gt_matrix, _, _ = load_dcm(gt_folder)
        _, DL_matrix, _, _ = load_dcm(dl_data_dir.format(case_name))
        _, DL_wo_matrix, _, _ = load_dcm(dl_wo_data_dir.format(case_name))
        # _, DL_t_matrix, _, _ = load_dcm(dl_t_data_dir.format(case_name))

        or_trans_diff, or_rot_diff = matrix_diff_analysis_2(gt_matrix.copy(), or_matrix.copy())
...
if __name__ == "__main__":
    ...
    for f, z in itertools.product(folds, test_id):
        b_path = 'results_ssim_f{}_{}_sup'.format(f, z)
